...
public class ItemRevolver extends ItemUpgradeableTool implements IShaderEquipableItem, ITool {
	private static final int FIRING_DELAY = 20;
	...
	public Slot[] getWorkbenchSlots(Container container, ItemStack stack, IInventory invItem) {
		return new Slot[] { new IESlot.Upgrades(container, invItem, 18 + 0, 80, 32, "REVOLVER", stack, true), new IESlot.Upgrades(container, invItem, 18 + 1, 100, 32, "REVOLVER", stack, true),
	...
	public void setShaderItem(ItemStack stack, ItemStack shader) {
		final ItemStack[] contained = this.getContainedItems(stack);
		contained[20] = shader;
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean adv) {
		if (stack.getItemDamage() != 1) {
			final String tag = getRevolverDisplayTag(stack);
			if (!tag.isEmpty())
			...
			final ItemStack shader = getShaderItem(stack);
			if (shader != null)
	...
	public ItemStack onItemRightClick(ItemStack revolver, World world, EntityPlayer player) {
		if (!world.isRemote) {
			if (player.isSneaking() || revolver.getItemDamage() == 1) {
				...
				final ItemStack[] bullets = getBullets(revolver);

				if (isEmpty(revolver)) {
					for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
						final ItemStack loader = player.inventory.getStackInSlot(i);
				...
				if (!ItemNBTHelper.getBoolean(revolver, "blocked") && this.cooldown <= 0) {
					if (bullets[0] != null && bullets[0].getItem() instanceof IBullet) {
						final IBullet bullet = (IBullet)bullets[0].getItem();
						
						if (bullet.canSpawnBullet(bullets[0])) {
							bullet.spawnBullet(player, bullets[0], getUpgrades(revolver).getBoolean("electro"));
							...
							world.playSoundAtEntity(player,"immersiveengineering:revolver_fire", 0.8f, 5f);
							this.cooldown = ItemRevolver.FIRING_DELAY;
						}
					} else {
						world.playSoundAtEntity(player, "note.hat", .6f, 3);
					}

					final ItemStack[] cycled = new ItemStack[getBulletSlotAmount(revolver)];
	...
	public boolean isEmpty(ItemStack stack) {
		final ItemStack[] bullets = getBullets(stack);
		boolean empty = true;
		for (ItemStack b : bullets)
	...
	public ItemStack[] getBullets(ItemStack revolver) {
		final ItemStack[] stackList = this.getContainedItems(revolver);
		final ItemStack[] bullets = new ItemStack[getBulletSlotAmount(revolver)];
	...
	public String getRevolverDisplayTag(ItemStack revolver) {
		final String tag = ItemNBTHelper.getString(revolver, "elite");
		if (!tag.isEmpty()) {
	...
	public String[] compileRender(ItemStack revolver) {
		...
		final String tag = ItemNBTHelper.getString(revolver, "elite");
		final String flavour = ItemNBTHelper.getString(revolver, "flavour");
		if (tag != null && !tag.isEmpty() && specialRevolversByTag.containsKey(tag)) {
			...
			final SpecialRevolver r = specialRevolversByTag.get(flavour);
			if (r != null && r.renderAdditions != null)
				for (String ss : r.renderAdditions)
					render.add(ss);
		}
		final NBTTagCompound upgrades = this.getUpgrades(revolver);
