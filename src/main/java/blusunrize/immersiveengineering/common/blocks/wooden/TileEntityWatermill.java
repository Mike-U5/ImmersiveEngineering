...
public class TileEntityWatermill extends TileEntityIEBase {
	...
	public void updateEntity() {
		...
		if (!multiblock && worldObj.getTotalWorldTime() % 256 == ((xCoord ^ zCoord) & 255)) {
			rotationVec = null;
		}
		...
		if (worldObj.getTileEntity(xCoord - fd.offsetX, yCoord, zCoord - fd.offsetZ) instanceof TileEntityDynamo && !multiblock) {
			...
			for (int l2 = 1; l2 < l; l2++) {
				...
				if (tileEntity instanceof TileEntityWatermill) {
					final TileEntityWatermill watermill = (TileEntityWatermill) tileEntity;
					watermill.rotation = rotation;
					watermill.canTurn = canTurn;
	...
	private boolean canUse(TileEntity tileEntity) {
		...
		return wm.offset[0] == 0 && wm.offset[1] == 0 && (wm.facing == facing || wm.facing == ForgeDirection.OPPOSITES[facing]) && !wm.isBlocked() && !wm.multiblock;
	}
	...
	public AxisAlignedBB getRenderBoundingBox() {
		if (renderAABB == null)
			if (offset[0] == 0 && offset[1] == 0)
				renderAABB = AxisAlignedBB.getBoundingBox(xCoord - (facing <= 3 ? 2 : 0), yCoord - 2, zCoord - (facing <= 3 ? 0 : 2), xCoord + (facing <= 3 ? 3 : 0), yCoord + 3,
						zCoord + (facing <= 3 ? 0 : 3));
			else
