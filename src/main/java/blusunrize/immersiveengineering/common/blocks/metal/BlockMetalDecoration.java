...
public class BlockMetalDecoration extends BlockIEBase implements blusunrize.aquatweaks.api.IAquaConnectable {
	...
	public boolean isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
		...
		if (meta == META_fence || meta == META_aluminiumFence)
			return side == UP;
		if (world.getBlockMetadata(x, y, z) == META_scaffolding || world.getBlockMetadata(x, y, z) == META_scaffolding2 || world.getBlockMetadata(x, y, z) == META_aluminiumScaffolding
	...
	public void registerBlockIcons(IIconRegister iconRegister) {
		...
		// icons[META_lantern][0] =
		// iconRegister.registerIcon("immersiveengineering:metalDeco_lantern_bottom");
	...
	public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
		...
		if (world.getBlockMetadata(x, y, z) == META_fence || world.getBlockMetadata(x, y, z) == META_aluminiumFence)
			this.setBlockBounds(canConnectFenceTo(world, x - 1, y, z) ? 0 : .375f, 0, canConnectFenceTo(world, x, y, z - 1) ? 0 : .375f, canConnectFenceTo(world, x + 1, y, z) ? 1 : .625f, 1,
			...
			if (arm.sideAttached > 0)
				this.setBlockBounds(f == 4 ? 0 : f == 5 ? .375f : .3125f, arm.inverted ? .3125f : 0, f == 2 ? 0 : f == 3 ? .375f : .3125f, f == 5 ? 1 : f == 4 ? .625f : .6875f,
						arm.inverted ? 1 : .6875f, f == 3 ? 1 : f == 2 ? .625f : .6875f);
			else
	...
	public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
		if (world.getBlock(x, y, z) == this) {
			int meta = world.getBlockMetadata(x, y, z);
			if (meta == META_fence || meta == META_aluminiumFence)
				this.setBlockBounds(canConnectFenceTo(world, x - 1, y, z) ? 0 : .375f, 0, canConnectFenceTo(world, x, y, z - 1) ? 0 : .375f, canConnectFenceTo(world, x + 1, y, z) ? 1 : .625f, 1.5f,
						canConnectFenceTo(world, x, y, z + 1) ? 1 : .625f);
	...
	public boolean shouldRenderFluid(IBlockAccess world, int x, int y, int z) {
		int meta = world.getBlockMetadata(x, y, z);
		return meta == META_fence || meta == META_lantern || meta == META_connectorStructural || meta == META_wallMount || meta == META_aluminiumFence;
	}
