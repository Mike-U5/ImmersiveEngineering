...
public class ItemRailgun extends ItemUpgradeableTool implements IShaderEquipableItem, IEnergyContainerItem, IZoomTool, ITool {
	...
	public void onUpdate(ItemStack stack, World world, Entity ent, int slot, boolean inHand) {
		// if(!world.isRemote && stack.getItemDamage()!=1 && ent!=null &&
		// ItemNBTHelper.hasKey(stack, "blocked"))
		// {
		// int l = ItemNBTHelper.handleDelayedSoundsForStack(stack, "casings", ent);
		// if(l==0)
		// ItemNBTHelper.setDelayedSoundsForStack(stack, "cylinderFill",
		// "tile.piston.in",.3f,3, 1,6,1);
		// l = ItemNBTHelper.handleDelayedSoundsForStack(stack, "cylinderFill", ent);
		// if(l==0)
		// ItemNBTHelper.setDelayedSoundsForStack(stack, "cylinderClose",
		// "fire.ignite",.6f,5, 1,6,1);
		// l = ItemNBTHelper.handleDelayedSoundsForStack(stack, "cylinderClose", ent);
		// if(l==0)
		// ItemNBTHelper.setDelayedSoundsForStack(stack, "cylinderSpin",
		// "note.hat",.1f,5, 5,8,1);
	...
	public int getMaxEnergyStored(ItemStack container) {
		return (8000 + this.getUpgrades(container).getInteger("capacity")) * 2;
	}
